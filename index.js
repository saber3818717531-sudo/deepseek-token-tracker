/**
 * 🐋 Token & Cache Monitor — FUSION build
 * ====================================================================
 * 融合 = [rskzayton/ST-TokenCacheMonitor 的分析面板/归一化/多模型/趋势图/每对话存储]
 *      + [自研抓取引擎：fetch+XHR 双拦截 + 放宽URL + 注入stream_options + 旁路克隆]
 *      + [消息下方挂账单] + [人民币计价]
 *
 * 为什么融合：原版 index.js 的 window.fetch 正则只认 /backends/... 代理路径，
 *   在「自定义 OpenAI 兼容端点直连」场景（如手机 TauriTavern 填 api.deepseek.com）
 *   拦不到请求 → 面板全 0。这里换上能覆盖直连+代理+XHR 的抓取引擎。
 */
import {
...
/** 每百万 tokens 价格。融合版统一用「人民币 ¥」显示，
 *  表内数值即 ¥/百万（已按你习惯的 DeepSeek 官方人民币价校准，其余模型按汇率折算）。 */
const PRICING = {
    'deepseek-v4-pro':   { input: 12.0,  cacheHit: 1.0,   output: 24.0  },
    'deepseek-v4-flash': { input: 1.0,   cacheHit: 0.02,  output: 2.0   }, // ← 你正在用的
...
const defaults = {
    ...
    showThroughput: true,
    showTrend:      true,
    showInChat:     true,    // ★ 融合新增：消息下方也挂一条账单
    verbose:        true,    // ★ 抓取诊断日志（确认拦到后再改 false）
    apiUrl:         'https://api.deepseek.com', // ★ 放宽匹配用
    costModel:      'deepseek-v4-flash',        // ★ 默认改成你在用的 flash
...
// ═══════════════════════════════════════════════════════════════════════
// Per-chat persistence (localStorage)
// ═══════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════
// usage 归一化（GitHub 版精华：统一 OpenAI/Claude/DeepSeek/Gemini）
// ═══════════════════════════════════════════════════════════════════════
function normalizeUsage(raw) {
    ...
    let normIn = input;
    // Claude 的 input_tokens 不含缓存，需补回
...
/** 从一段 SSE/JSON 文本尽力抠 usage（流式从后往前找含 usage 的 data 行） */
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        ...
        try {
            ...
            if (u && (u.prompt_tokens || u.input_tokens || u.completion_tokens || u.output_tokens)) {
                merged = { ...(merged || {}), ...u };   // 流式末帧才带完整 usage，合并兜底
...
// ═══════════════════════════════════════════════════════════════════════
// ★ 自研抓取引擎：放宽URL + 注入stream_options + 旁路克隆（fetch & XHR 双拦截）
// ═══════════════════════════════════════════════════════════════════════
/** 既认代理路径，也认直连域名 / chat/completions / /v1/ —— 覆盖你手机直连场景 */
function isChatUrl(url) {
    if (!url) return false;
    const u = String(url).toLowerCase();
    const custom = (cfg.apiUrl || '').toLowerCase().replace(/\/+$/, '');
    return /\/(backends\/(chat|text)-completions|generate)/i.test(u)   // 代理路径（GitHub 原版）

        || u.includes('chat/completions') || u.includes('/v1/')        // 直连/兼容端点（自研放宽）
...
function handleUsageFromText(text, startTs) {
    ...
    if (!u || u.total <= 0) { log('⚠️ 响应里没找到 usage（若长期如此，见 README 装 server-patch）'); return; }
...
function bindUI() {
    ...
    head?.addEventListener('mousedown', e => { if (e.target.tagName === 'BUTTON') return; dragging = true; const r = root.getBoundingClientRect(); dX = e.clientX - r.left; dY = e.clientY - r.top; root.style.cursor = 'grabbing'; e.preventDefault(); });
    document.addEventListener('mousemove', e => { if (!dragging) return; cfg.panelPosition.x = e.clientX - dX; cfg.panelPosition.y = e.clientY - dY; root.style.right = 'auto'; root.style.bottom = 'auto'; root.style.left = cfg.panelPosition.x + 'px'; root.style.top = cfg.panelPosition.y + 'px'; });
    document.addEventListener('mouseup', () => { if (!dragging) return; dragging = false; root.style.cursor = ''; saveCfg(); });
...
// —— 设置弹窗（GitHub 版 + 融合新增项：消息挂账/verbose/匹配URL/人民币自定义）——
...
function init() {
    loadCfg();
    try { _currentChatId = getContext()?.name2 || 'default'; } catch { _currentChatId = 'default'; }
    loadSession(); hookEvents(); createUI(); registerSlashCommands();
    log('🐋 融合版就绪 | 模型', cfg.costModel, '| 抓取: fetch+XHR 双拦截已挂载 | 发消息后看 🔎 日志');
}
